...
describe("runOpenClawCliRequest", () => {
  ...
  test("uses recent Slack context to resolve Jira user lookup follow-ups", async () => {
            ...
            recentMessages: [
              {
                author: "user",
                speaker: "<@U456>",
                text: "which Jira tickets did I assign to Boris Renski?"
              }
            ]
          },
          connections: {
            ...
            content: [
              {
                ...
                displayName: "Boris Renski",
                emailAddress: "brenski@apegpt.ai"
              }
            ]
          };
        }
      ...
      async (_command, args) => {
        ...
        return prompts.length === 1
          ...
          : {
              exitCode: 0,
              stdout:
                "Boris Renski resolved to Jira account `acct-boris` (`brenski@apegpt.ai`).",
              stderr: ""
            };
      },
      () => undefined
    ...
    expect(prompts[0]).toContain("Boris Renski");
  ...
  test("lets OpenClaw chain multiple Atlassian MCP calls for a Jira action", async () => {
    ...
    const response = await runOpenClawCliRequest(
      {
        input: {
          text: "create new Jira ticket in DM workspace using Atlassian MCP, titled 'test ticket from slack' and assign it to Boris Renski (brenski@apegpt.ai)",
          connections: {
            github: { connected: false },
            jira: {
              connected: true,
