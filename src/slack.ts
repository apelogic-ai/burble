...
function stripRuntimeToolCallProtocolFragments(text: string): string {
  let output = "";
  let index = 0;
  ...
  while (index < text.length) {
    ...
    const end = findRuntimeJsonObjectEnd(text, index);
    if (end === null) {
      output += text[index];
      index += 1;
      continue;
    ...
    const parsed = parseRuntimeJsonObject(candidate);
...
function findRuntimeJsonObjectEnd(text: string, start: number): number | null {
  let depth = 0;
  let inString = false;
  let escaped = false;
