...
function formatActiveConversationRouteInstruction(
  ...
  return [
    ...
    'If the user names a different Slack destination such as "#eng", "<#C123|eng>", or a channel id, do not use the active conversation route and do not put the Slack label in delivery.to. For output that only reads public/open-internet sources, first call scheduledJob.registerCapability with destination set to that label and visibilityPolicy {"maxOutputVisibility":"public"}, then set delivery.to to the returned convrt_* route. If the job reads from authenticated Burble provider sources, do not register the channel destination; report that explicit declassification approval is not implemented. If registration does not return ok with a resolved route, do not update, enable, or trigger the job.',
