...
function formatScheduledJobContextInstruction(request: RunRequest): string[] {
  ...
  lines.push(
    ...
    "Respect maxOutputVisibility when sending scheduled output. Do not publicly post private-tool-derived content; public channel delivery for authenticated provider output requires an explicit declassification approval flow that is not implemented yet."
  );
...
function formatScheduledProviderCapabilityInstruction(
  ...
  return [
    ...
    "If the scheduled job uses authenticated Burble provider tools such as GitHub, Google, Jira, Slack search, HubSpot, or Atlassian MCP, do not register a Slack channel destination. Report that public channel delivery for private-tool output requires an explicit declassification approval flow that is not implemented yet.",
    'If the scheduled job only uses public/open-internet sources and Burble channel delivery, register it with requiredTools ["conversation.sendMessage"], destination set to the requested channel, and visibilityPolicy {"maxOutputVisibility":"public"}.',
...
function formatActiveConversationRouteInstruction(
  ...
  return [
    ...
    'If the user names a different Slack destination such as "#eng", "<#C123|eng>", or a channel id, do not use the active conversation route and do not put the Slack label in delivery.to. For public/open-internet scheduled output, first call scheduledJob.registerCapability with destination set to that label and visibilityPolicy {"maxOutputVisibility":"public"}, then set delivery.to to the returned convrt_* route. If the job uses authenticated Burble provider tools, do not register the channel destination; report that explicit declassification approval is not implemented. If registration does not return ok with a resolved route, do not update, enable, or trigger the job.',
